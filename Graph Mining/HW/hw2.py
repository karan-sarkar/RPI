...
import random
'''
1a) Simulation 2 has less deaths that simulation 1. THis is not surprising because the high traffic pathways are now immune greatly 
reducing disease spread. Sim 3 have even less deaths. This is surprising because we have more infection on central nodes. THis makes 
sense because the infection is basically affecting hubs while immunity is affecting high traffic areas. THus, it prevents the
disease from metastizing. Killing the hubs early allows more to survive.

1b) You could add immunity for those who have survived the disease before and a recovery probability.

2a) Adamic showed the 

2b) THis is different from the DNC data. The DNC data had much more accurate results because of natural structure.

2c) In the DNC data the triad closure holds. THis is because conversations often involve mltiple people. THus we got getter results.

2d) I would use some sort of supervised learning to get better results. This would work by training the data on a hge dataset.
...
for i in range(int(len(amazon_edges)/4)):
    edge = amazon_edges[i]
    for i in range(0, 2):
...
print(len(amazon_graph_partial.nodes()))
...
for node in amazon_friends.nodes:
    ...
    p_preds = p_preds[0: int(len(p_preds) / 4) + 1]
    p_total += len(p_preds)
    for p_pred in p_preds:
        if p_pred in amazon_graph_full.neighbors(node):
            p_total += 1
            
    t_preds = sorted(t_preds,key=lambda x: -1 * x[1])
    t_preds = t_preds[0: int(len(t_preds) / 4) + 1]
    t_total += len(t_preds)
    for t_pred in t_preds:
        if t_pred in amazon_graph_full.neighbors(node):
            t_total += 1

print(c_matches,c_total)
print(a_matches,a_total)
print(p_matches,p_total)
print(t_matches,t_total)
